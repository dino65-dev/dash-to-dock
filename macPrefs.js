...
export function addMacOSPreferencesPage(extensionPreferences, notebook) {
    ...
    installOuterPageScrollRouting(page);

    page.append(makeSectionLabel('macOS Native Dock'));
    page.append(makeSwitchRow(settings, 'macos-style', 'Enable macOS renderer',
        'Use the high-resolution compositor renderer while keeping Dash-to-Dock as the backend.'));

    page.append(makeSectionLabel('Magnification & Motion'));
    page.append(makeDoubleScaleRow(settings, 'macos-magnification', 'Peak magnification',
        'Extra scale applied under the pointer.', 0, 1.25, 0.05,
        value => `${(1 + value).toFixed(2)}×`));
    page.append(makeDoubleScaleRow(settings, 'macos-magnification-radius', 'Fish-eye radius',
        'Distance over which neighboring icons join the wave.', 48, 360, 2,
        value => `${Math.round(value)} px`));
    page.append(makeDoubleScaleRow(settings, 'macos-spring-response', 'Spring response',
        'Higher values make the dock follow the pointer more quickly.', 8, 50, 1,
        value => value.toFixed(0)));
    page.append(makeDoubleScaleRow(settings, 'macos-spring-damping', 'Spring damping',
        '1.0 is critically damped; lower values add a little overshoot.', 0.5, 1.5, 0.05,
        value => value.toFixed(2)));
    page.append(makeDoubleScaleRow(settings, 'macos-icon-quality', 'Icon texture quality',
    ...
    page.append(makeDoubleScaleRow(settings, 'macos-light-opacity', 'Light material opacity',
        'Opacity used for the frosted light-gray material.', 0.1, 0.9, 0.02,
        value => `${Math.round(value * 100)}%`));
    page.append(makeDoubleScaleRow(settings, 'macos-dark-opacity', 'Dark material opacity',
        'Opacity used for the translucent charcoal material.', 0.1, 0.95, 0.02,
        value => `${Math.round(value * 100)}%`));
    page.append(makeDoubleScaleRow(settings, 'macos-glass-opacity', 'Fallback material opacity',
    ...
    page.append(makeSwitchRow(settings, 'macos-dynamic-border', 'Dynamic border',
        'Use a sharp dark edge in light mode and a sharp light edge in dark mode.'));
    page.append(makeDoubleScaleRow(settings, 'macos-border-opacity', 'Border contrast',
        'Strength of the adaptive one-pixel outline.', 0, 0.7, 0.02,
        value => `${Math.round(value * 100)}%`));
    page.append(makeSwitchRow(settings, 'macos-reflection', 'Reflective shelf',
        'Show a subtle mirrored icon reflection when the dock is at the bottom of the screen.'));
    page.append(makeDoubleScaleRow(settings, 'macos-reflection-opacity', 'Reflection opacity',
        'Strength of the mirrored icon image.', 0, 0.4, 0.01,
        value => `${Math.round(value * 100)}%`));
    page.append(makeDoubleScaleRow(settings, 'macos-reflection-height', 'Reflection depth',
...
function makeSwitchRow(settings, key, title, subtitle) {
    ...
    const toggle = new Gtk.Switch({valign: Gtk.Align.CENTER, halign: Gtk.Align.END});
...
function makeDoubleScaleRow(settings, key, title, subtitle, min, max, step, formatter) {
    return makeScaleRow(settings, key, title, subtitle, min, max, step, formatter, false);
}

function makeIntScaleRow(settings, key, title, subtitle, min, max, step, formatter) {
    return makeScaleRow(settings, key, title, subtitle, min, max, step, formatter, true);
}

function makeScaleRow(settings, key, title, subtitle, min, max, step, formatter, integer) {
    const wrapper = new Gtk.Box({orientation: Gtk.Orientation.VERTICAL, spacing: 5});
    const header = makeRow(title, subtitle);
    const valueLabel = new Gtk.Label({xalign: 1, width_chars: 8});
...
function installOuterPageScrollRouting(page) {
    ...
    controller.connect('scroll', (scroll, _dx, dy) => {
        ...
        const scroller = findAncestorScrolledWindow(page);
        if (!scroller)
            return false;

        const adjustment = scroller.get_vadjustment();
        const lower = adjustment.get_lower();
        const maximum = Math.max(lower,
            adjustment.get_upper() - adjustment.get_page_size());
        const delta = scroll.get_unit?.() === Gdk.ScrollUnit.SURFACE
            ? Math.max(-96, Math.min(96, dy))
            : Math.sign(dy) * Math.max(42, adjustment.get_step_increment());
        adjustment.set_value(Math.max(lower, Math.min(maximum,
...
function findAncestorScrolledWindow(widget) {
